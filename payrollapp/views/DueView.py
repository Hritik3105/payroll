    ...
    if request.method =="POST":
        ...
        if len(val)==0: 
            ...
            for i in empexceldata.itertuples():
                ...
                val=Providers.objects.filter(id=request.user.id).update(issue_date=d,provider_name=i.RUTProveedor,invoice=i.Folio,business_name=i.RazonSocial,total_amount_paid=i.MontoTotal,expiration_date=exp,payment_week=4,month_of_payment=months,year_of_payment=years,days_overdue=30)
        return redirect("due")
